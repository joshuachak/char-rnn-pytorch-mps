"""
GRU model implementation for character-level language modeling.

This module provides a PyTorch implementation of multi-layer GRU
...
class CharGRU(nn.Module):
    """
    Multi-layer GRU for character-level language modeling.
    
    This implementation provides:
    - Multi-layer GRU with configurable hidden size and number of layers
    ...
    def _init_weights(self):
        ...
        # Initialize GRU weights with uniform distribution [-0.08, 0.08]
    ...
    def forward(self, x, hidden=None):
        ...
        # Pass through GRU
        gru_out, hidden = self.gru(embedded, hidden)  # (batch_size, seq_length, hidden_size)
        
        # Project to vocabulary size
        output = self.output_proj(gru_out)  # (batch_size, seq_length, vocab_size)
